    ...
    <div id="content" class="content">
        ...
        <div class="panel panel-inverse">
            ...
            @foreach($services as $service)
                    ...
                    <tr class="gradeU" id="post_{!! $service->id !!}">
                        <td class="f-s-600 text-inverse">{!! $service->id !!}</td>
                        <td class="with-img"><img src="{!! asset('/web/services/'.$service->img_url) !!}" class="img-rounded height-30" alt=""/></td>
                        <td><a href="{!! route('admin.service.edit',$service->id) !!}">{!! $service->title !!} - <small>Düzenlemek için tıklayınız</small></a></td>
                        <td>
                            <div class="switcher switcher-success">
                                <input type="checkbox" name="status" id="status{!! $service->id !!}" class="changeStatus" {!! ($service->status ==  1) ? 'checked' : null !!} value="{!! $service->status !!}" data-id="{!! $service->id !!}" data-status="{!! $service->status !!}">
                                <label for="status{!! $service->id !!}"></label>
                            </div>
                        </td>
                    </tr>
            @endforeach
...
@section('script')
    ...
    <script>
        ...
        $(document.body).on('click',changeStatusButton,function () {
            ...
            $.ajax({
                url:"{!! route('admin.service.statusUpdate') !!}",
                type: 'POST',
                data : {
