    ...
    <div id="content" class="content">
        ...
        <div class="panel panel-inverse">
            ...
            @foreach($testimonials as $testimonial)
                    ...
                    <tr class="gradeU" id="post_{!! $testimonial->id !!}">
                        <td class="f-s-600 text-inverse">{!! $testimonial->id !!}</td>
                        <td class="with-img"><img src="{!! asset('/web/testimonials/'.$testimonial->img_url) !!}" class="img-rounded height-30" alt=""/></td>
                        <td><a href="{!! route('admin.testimonials.edit',$testimonial->id) !!}">{!! $testimonial->name !!} - <small>Düzenlemek için tıklayınız</small></a></td>
                        <td>
                            <div class="switcher switcher-success">
                                <input type="checkbox" name="status" id="status{!! $testimonial->id !!}" class="changeStatus" {!! ($testimonial->status ==  1) ? 'checked' : null !!} value="{!! $testimonial->status !!}" data-id="{!! $testimonial->id !!}" data-status="{!! $testimonial->status !!}">
                                <label for="status{!! $testimonial->id !!}"></label>
                            </div>
                        </td>
                        <td>
                            <a class="btn btn-danger" href="{!! route('admin.testimonials.destroy',$testimonial->id) !!}"><i class="fa fa-trash"></i></a>
                            <a class="btn btn-warning" href="{!! route('admin.testimonials.edit',$testimonial->id) !!}"><i class="fa fa-pen"></i></a>
                        </td>
                    </tr>
            @endforeach
...
@section('script')
    ...
    <script>
        ...
        $(document.body).on('click',changeStatusButton,function () {
            ...
            $.ajax({
                url:"{!! route('admin.testimonials.statusUpdate') !!}",
                type: 'POST',
                data : {
                ...
                success:function (response){
                    ...
                    if (response == true){
                        admin_respones('Başarıyla Güncellendi','Yorum Güncelleme İşlemi Başarılı Oldu','success');
                    }else{
                        admin_respones('Başarısız Oldu','Yorum Güncelleme İşlemi Başarısız Oldu','error');
                    }
