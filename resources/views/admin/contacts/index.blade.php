        ...
        <div class="panel panel-inverse">
            ...
            <div class="panel-body">
                <table id="data-table-default" class="table table-striped table-bordered table-td-valign-middle">
                    ...
                    <tr>
                        <th width="1%">İD</th>
                        <th class="text-nowrap">Gönderen</th>
                        <th class="text-nowrap">Mail</th>
                        <th class="text-nowrap">Görüntüle</th>
                    </tr>
                    </thead>
                    <tbody>
            ...
            @foreach($contacts as $contact)
                    ...
                    <tr class="gradeU" id="post_{!! $contact->id !!}">
                        <td class="f-s-600 text-inverse">{!! $contact->id !!}</td>
                        <td><a href="{!! route('admin.contact.show',$contact->id) !!}">{!! $contact->name !!}</a></td>
                        <td>{!! $contact->mail !!}</td>
                        <td><a href="{!! route('admin.contact.show',$contact->id) !!}" class="btn btn-success btn-block">Görüntüle</a></td>
...
@section('script')
    ...
    <script>
        ...
        $(document.body).on('click',changeStatusButton,function () {
            ...
            $.ajax({
                url:"{!! route('admin.contact.statusUpdate') !!}",
                type: 'POST',
                data : {
                ...
                success:function (response){
                    ...
                    if (response == true){
                        admin_respones('Başarıyla Güncellendi','Mesaj Güncelleme İşlemi Başarılı Oldu','success');
                    }else{
                        admin_respones('Başarısız Oldu','Mesaj Güncelleme İşlemi Başarısız Oldu','error');
                    }
